// 거래대행 수수료/배달료 계산기 + 청구금액 내역 — 외부 link/내부 draft 공통
//
// 외부 link 흐름  : pickupAddr + deliveryAddr 모두 있음 → 거리 자동 계산 + 배달비/총액 미리보기
// 내부 draft 흐름 : pickupAddr 만 있음 (구매자 수령지는 PATCH /buyer-info 에서) → 배달비 미산정 안내
//
// 옵션 enum 은 라운드13 정정 소문자 — types.ts 의 WeightKey/VolumeKey/FragilityKey 와 동일.
...
export function calcFees(
  ...
  const commissionFee = Math.floor(itemPrice * s.commissionRate)

  let deliveryFee: number
  if (isVan) {
    const mpkm = s.baseFuelPrice / s.truckFuelEfficiency
    const fc = s.fuelPricePerL / s.baseFuelPrice
    const kmRate = s.truckBaseKmRate - mpkm + mpkm * fc
    const raw = (s.truckBaseDeliveryFee + kmRate * distKm) * fragMul
    deliveryFee = Math.round(Math.max(s.truckMinDeliveryFee, Math.round(raw / 100) * 100))
  } else {
    const mpkm = s.baseFuelPrice / s.fuelEfficiency
    const fc = s.fuelPricePerL / s.baseFuelPrice
    const kmRate = s.baseKmRate - mpkm + mpkm * fc
    const raw = (s.baseDeliveryFee + kmRate * distKm) * wtMul * vlMul * fragMul
    deliveryFee = Math.round(Math.max(s.minDeliveryFee, Math.round(raw / 100) * 100))
  }
...
interface Props {
  ...
  itemPrice:    number          // 0 이면 가격 라인 미표시 (외부 link EXTERNAL 모드)
  pickupAddr:   LatLng | null   // 외부 link / 내부 draft 둘 다 필수
  deliveryAddr: LatLng | null   // 내부 draft 흐름은 null

  settings: EscrowFeeSettings | undefined  // 백엔드 fee_settings 응답
}

export default function FeeCalculator({
  weight, volume, fragility,
  onWeightChange, onVolumeChange, onFragilityChange,
  itemPrice, pickupAddr, deliveryAddr,
  settings,
}: Props) {
  ...
  // fee — 옵션 + 거리 + 설정 모두 있을 때만 산정
  const fees: FeeBreakdown | null = useMemo(() => {
    if (!settings || !wOpt || !vOpt || !fOpt || distanceKm == null) return null
    return calcFees(itemPrice, distanceKm, wOpt.multiplier, vOpt.multiplier, fOpt.multiplier, isVan, settings)
  }, [settings, wOpt, vOpt, fOpt, distanceKm, itemPrice, isVan])
    ...
    <div className="flex flex-col gap-6">
        ...
        <div className="flex justify-between items-center px-3 py-2.5 bg-white rounded-lg border border-gray-200 text-sm">
          ...
          {distanceKm != null
            ...
            : <span className="text-xs text-gray-400">
                {pickupAddr ? '수령지 입력 후 산정' : '픽업·수령지 선택 후 산정'}
              </span>
          }
        </div>
      ...
      <section className="bg-white rounded-xl border border-gray-200 p-4 sm:p-5">
        ...
        <div className="space-y-2.5 text-sm">
          ...
          <div className="flex justify-between text-gray-600">
            <span>대행 수수료 {settings && `(${(settings.commissionRate * 100).toFixed(0)}%)`}</span>
