...
import { Package, Search, Eye, Heart, AlertTriangle, Trash2, Hash, ExternalLink } from 'lucide-react'
import { Link } from 'react-router-dom'
import { toast } from 'sonner'
import { useAdminItems, useAdminItemDetail } from '@/features/admin/hooks'
...
export default function AdminItemPage() {
  ...
  const handleSearch = () => { setQ(qInput.trim()); setPage(0) }
    ...
    <div className="pb-10">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-3 mb-4 flex flex-col gap-2">
        ...
        <div className="flex gap-2 flex-wrap text-xs">
          <FilterSelect value={status} onChange={(v) => { setStatus(v as ItemStatus | 'ALL'); setPage(0) }} options={STATUS_OPTIONS} />
          <FilterSelect value={tradeType} onChange={(v) => { setTradeType(v as TradeType | 'ALL'); setPage(0) }} options={TRADE_TYPE_OPTIONS} />
          <FilterSelect value={sort ?? 'latest'} onChange={(v) => { setSort(v as AdminItemsListParams['sort']); setPage(0) }} options={SORT_OPTIONS} />
        </div>
      </div>
      ...
      {detailId != null && (
        <AdminItemDetailModal id={detailId} onClose={() => setDetailId(null)} />
      )}
    </div>
  )
}
...
}: { value: string; onChange: (v: string) => void; options: readonly { value: string; label: string }[] }) {
  return (
    <select
      value={value}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5 flex-wrap">
          ...
          <span className={cn(
            ...
            item.status === '판매중' ? 'bg-green-100 text-green-700' :
            item.status === '예약'   ? 'bg-yellow-100 text-yellow-700' :
            item.status === '거래완료' ? 'bg-gray-100 text-gray-600' :
            'bg-red-100 text-red-700',
          )}>
            {item.status}
          </span>
          {item.tradeTypes?.map((t) => (
            <span key={t} className="text-[10px] px-1.5 py-0.5 rounded font-medium bg-blue-50 text-blue-700">
              {t}
            </span>
          ))}
        </div>
        ...
        <div className="flex items-center gap-3 text-[11px] text-gray-400 flex-wrap">
          <span className="inline-flex items-center gap-0.5"><Eye size={11} /> {item.viewCount}</span>
          <span className="inline-flex items-center gap-0.5"><Heart size={11} /> {item.wishlistCount}</span>
          {item.reportCount > 0 && (
        ...
        {isLoading || !data ? (
          ...
          <>
            <div className="flex items-start justify-between mb-3">
              ...
              <button onClick={onClose} className="text-gray-400 hover:text-gray-600 px-2 text-xl leading-none">×</button>
            </div>
            ...
            <section className="mb-4">
              <p className="text-xs font-semibold text-gray-700 mb-2">신고 이력 ({data.reportHistory.length})</p>
              {data.reportHistory.length === 0 ? (
                <p className="text-xs text-gray-400">신고 없음</p>
              ) : (
                <ul className="flex flex-col gap-1.5">
                  {data.reportHistory.map((r) => (
                    <li key={r.id} className="text-xs text-gray-700 border border-gray-100 rounded-lg px-2.5 py-1.5">
            ...
            <section className="mb-4">
              <p className="text-xs font-semibold text-gray-700 mb-2">거래 이력 ({data.transactionHistory.length})</p>
              {data.transactionHistory.length === 0 ? (
                <p className="text-xs text-gray-400">거래 없음</p>
              ) : (
                <ul className="flex flex-col gap-1.5">
                  {data.transactionHistory.map((t) => (
                    <li key={t.id} className="text-xs text-gray-700 border border-gray-100 rounded-lg px-2.5 py-1.5 flex justify-between gap-2">
                      <span className="truncate">
                        #{t.id} · 구매자{' '}
                        <Link to={`/users/${t.buyerId}`} className="text-primary-600 hover:underline">
                          {t.buyerNickname}
                        </Link>
                        {' · '}{t.status} · {t.price.toLocaleString()}원
                      </span>
                      {t.completedAt && <span className="text-gray-400 shrink-0">{formatKst(t.completedAt, 'M.d HH:mm')}</span>}
                    </li>
                  ))}
                </ul>
              )}
            ...
            <div className="flex gap-2 pt-3 border-t border-gray-100">
              ...
              <Button variant="danger" fullWidth onClick={() => setConfirmDel(true)} disabled={isDeleting}>
...
function Stat({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className={cn(
      ...
      <p className={cn('text-sm font-bold', highlight ? 'text-red-600' : 'text-gray-900')}>{value}</p>
    </div>
  )
}
